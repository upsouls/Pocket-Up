-- header - заголовок; placeholder - подсказка для заполнения текстового поля; isCorrectValue - не обязательно для заполнения. Это функция. при вызове проекряет на корректность заполненгое поле. если поле корректно - вернется "", если нет - "название ошибки"; value - необязательное поле для заполнения. при старте установит значение  в текстовое поле; funEditingEnd - функция, которая вызывается когда пользователь написал ответ и отправил его;
