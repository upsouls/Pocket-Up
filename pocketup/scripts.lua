...
--checkboxLocal:setFillColor(171/255, 219/255, 241/255)
...
funAddBlock = function (blockTable)
    ...
    local function fCycleForever()
        local block = createBlock({"endCycleForever"})
        for i=1, #block.cells do
            block.cells[i][2]:addEventListener("touch",touchParameter)
        end
        groupSceneScroll:insert(block)
        block.alpha = 0
        return({{{"endCycleForever",{},"on"}, block}})
    end
    local function fIfElse()
        local block = createBlock({"else"})
        for i=1, #block.cells do
            block.cells[i][2]:addEventListener("touch",touchParameter)
        end
        local block2 = createBlock({"endIf"})
        for i=1, #block2.cells do
            block2.cells[i][2]:addEventListener("touch",touchParameter)
        end
        groupSceneScroll:insert(block)
        groupSceneScroll:insert(block2)
        block.alpha = 0
        block2.alpha = 0
        return({ {{"else",{},"on"},block},{{"endIf",{},"on"}, block2}})
    end
    local function fIf()
        local block = createBlock({"endIf"})
        for i=1, #block.cells do
            block.cells[i][2]:addEventListener("touch",touchParameter)
        end
        groupSceneScroll:insert(block)
        block.alpha = 0
        return({ {{"endIf",{},"on"},block}})
    end
    local function fRepeat()
        local block = createBlock({"endRepeat"})
        for i=1, #block.cells do
            block.cells[i][2]:addEventListener("touch",touchParameter)
        end
        groupSceneScroll:insert(block)
        block.alpha = 0
        return({ {{"endRepeat",{},"on"},block}})
    end
    local function fFor()
        local block = createBlock({"endFor"})
        for i=1, #block.cells do
            block.cells[i][2]:addEventListener("touch",touchParameter)
        end
        groupSceneScroll:insert(block)
        block.alpha = 0
        return({ {{"endFor",{},"on"},block}})
    end
    local function fForeach()
        local block = createBlock({"endForeach"})
        for i=1, #block.cells do
            block.cells[i][2]:addEventListener("touch",touchParameter)
        end
        groupSceneScroll:insert(block)
        block.alpha = 0
        return({ {{"endForeach",{},"on"},block}})
    end
    local function fTimer()
        local block = createBlock({"endTimer"})
        for i=1, #block.cells do
            block.cells[i][2]:addEventListener("touch",touchParameter)
        end
        groupSceneScroll:insert(block)
        block.alpha = 0
        return({ {{"endTimer",{},"on"},block}})
    end
    local function fWait()
        local block = createBlock({"endWait"})
        for i=1, #block.cells do
            block.cells[i][2]:addEventListener("touch",touchParameter)
        end
        groupSceneScroll:insert(block)
        block.alpha = 0
        return({ {{"endWait",{},"on"},block}})
    end
    local tableInvestBlocks = {
        ["cycleForever"]=fCycleForever,
        ["ifElse (2)"]=fIfElse,
        ["if (2)"]=fIf,
        ["repeat"]=fRepeat,
        ["repeatIsTrue"]=fRepeat,
        ["for"]=fFor,
        ["foreach"]=fForeach,
        ["timer"]=fTimer,
        ["waitIfTrue"]=fWait
    ...
    block.investBlocks = allBlocks[originalName][1]=="event" and {} or tableInvestBlocks[originalName]~=nil and tableInvestBlocks[originalName]() or nil
    groupSceneScroll:insert(block)
