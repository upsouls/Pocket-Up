...
function scene_menu()
    ...
    local topBarArray = topBar(groupScene, 'Pocket Up', funMenuObjects, nil, nil)
    topBarArray[2].fill = {
        type='image',
    ...
    local texts = {
        ...
        --{text='Paint Up', icon='images/paint.png'}
    }
    ...
    local y = display.actualContentWidth/(#texts/1.5)/2+55
    for i = 1, #texts do
        if i < 2 then
            local a = display.newImageRect('images/menubutton.jpg', display.actualContentWidth, display.actualContentWidth/(#texts/1.5)+100)
            ...
            local textRecentProject = display.newText(textRecentProjectShadow.text, 0, 0, nil, 30)
            print(display.actualContentWidth/20)
            if string.len(recentProjectName) > display.actualContentWidth/20 then
                textRecentProjectShadow.text = string.sub(textRecentProjectShadow.text, 1, display.actualContentWidth/20)..'...'
                textRecentProject.text = textRecentProjectShadow.text
            end
            ...
            local a = display.newImageRect('images/menubutton.jpg', display.actualContentWidth, display.actualContentWidth/(#texts/1.5))
            scrollItems:insert(a)
            a.y = y
            a.x = CENTER_X
            local icon = display.newImageRect(texts[i].icon, 128, 128)
